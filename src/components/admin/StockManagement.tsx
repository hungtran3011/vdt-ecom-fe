...
export default function StockManagement({ 
  ...
  const handleStockAdjustment = async () => {
    ...
    const validationResult = validateForm(adjustmentData, {
      productId: asValidator(StockValidators.stockMovement.productId),
      type: asValidator(StockValidators.stockMovement.type),
      quantity: asValidator(StockValidators.stockMovement.quantity),
      reason: asValidator(StockValidators.stockMovement.reason)
    });
    ...
    if (!validationResult.isValid) {
      const errorMessage = Object.values(validationResult.errors)
        .flat()
        .join(', ');
      showSnackbar(`${t('form.validation.error')}: ${errorMessage}`, 'error');
      return;
    }

    // Additional business logic validation
    if (adjustmentType === StockMovementType.OUT && 
        adjustmentData.quantity > selectedItem.availableStock) {
      showSnackbar(t('stock.cannotRemoveMoreThanAvailable'), 'error');
      return;
    }

    try {
      await createStockMovement.mutateAsync(adjustmentData);
  ...
  const handleAcknowledgeAlert = async (alertId: number) => {
    if (!alertId || alertId <= 0) {
      showSnackbar('Invalid alert ID', 'error');
      return;
    }
    ...
    try {
      ...
      const errorMessage = formatApiError(error);
      showSnackbar(`Failed to acknowledge alert: ${errorMessage}`, 'error');
  ...
  const handleExportReport = async (format: 'csv' | 'xlsx') => {
    if (!format || !['csv', 'xlsx'].includes(format)) {
      showSnackbar('Invalid export format', 'error');
      return;
    }

    try {
      await exportReport.mutateAsync(format);
      showSnackbar(`Stock report exported as ${format.toUpperCase()} successfully`, 'success');
    } catch (error) {
      const errorMessage = formatApiError(error);
        ...
        <div>
          ...
          {stockSummary && (
            <p className="text-(--md-sys-color-on-surface-variant) mt-1">
              {stockSummary.totalProducts} products • ${stockSummary.totalStockValue.toFixed(2)} total value
            </p>
          )}
        </div>
          ...
          <Card className="w-full max-w-md">
            <div className="p-6">
              ...
              <div className="flex justify-end gap-3 mt-6">
                ...
                <Button
                  variant="filled"
                  label={createStockMovement.isPending ? t('stock.processing') : t('stock.adjustStock')}
                  onClick={handleStockAdjustment}
                  disabled={createStockMovement.isPending || !adjustmentQuantity || !adjustmentReason}
                />
              </div>
            </div>
          </Card>
